let NodeDish,NodeRecipe,OperationConfig,sanitise,removeSubheadingsFromArray,sentenceToCamelCase,ExcludedOperationError;_bba‍.x([["_wrap",()=>_wrap],["help",()=>help],["bake",()=>bake],["_explainExcludedFunction",()=>_explainExcludedFunction]]);_bba‍.w("./NodeDish.mjs",[["default",["NodeDish"],function(v){NodeDish=v}]]);_bba‍.w("./NodeRecipe.mjs",[["default",["NodeRecipe"],function(v){NodeRecipe=v}]]);_bba‍.w("../core/config/OperationConfig.json",[["default",["OperationConfig"],function(v){OperationConfig=v}]]);_bba‍.w("./apiUtils.mjs",[["sanitise",["sanitise"],function(v){sanitise=v}],["removeSubheadingsFromArray",["removeSubheadingsFromArray"],function(v){removeSubheadingsFromArray=v}],["sentenceToCamelCase",["sentenceToCamelCase"],function(v){sentenceToCamelCase=v}]]);_bba‍.w("../core/errors/ExcludedOperationError.mjs",[["default",["ExcludedOperationError"],function(v){ExcludedOperationError=v}]]);/**
...
function transformArgs(opArgsList, newArgs) {
    ...
    const opArgs = Object.assign([], opArgsList).map((a) => {
        if (Array.isArray(a.value)) {
            a.value = _bba‍.a("removeSubheadingsFromArray",removeSubheadingsFromArray)(a.value);
        }
        return a;
    });
...
function ensureIsDish(input) {
    if (!input) {
        return new (_bba‍.a("NodeDish",NodeDish))();
    }

    if (input instanceof _bba‍.a("NodeDish",NodeDish)) {
        return input;
    } else {
        return new (_bba‍.a("NodeDish",NodeDish))(input);
    }
}
    ...
    op.args.forEach((a) => {
        if (a.type === "option" || a.type === "editableOption") {
            result[sentenceToCamelCase(a.name)] = {
                type: a.type,
                options: _bba‍.a("removeSubheadingsFromArray",removeSubheadingsFromArray)(a.value)
            };
        } else if (a.type === "toggleString") {
            result[sentenceToCamelCase(a.name)] = {
                type: a.type,
                value: a.value,
                toggleValues: _bba‍.a("removeSubheadingsFromArray",removeSubheadingsFromArray)(a.toggleValues),
            };
        } else {
            result[sentenceToCamelCase(a.name)] = {
                type: a.type,
 ...
 */
    ...
    // matches first.
    const matches = Object.keys(_bba‍.a("OperationConfig",OperationConfig))
        // hydrate operation: swap op name for op config object (with name)
        .map((m) => {
            const hydrated = _bba‍.a("OperationConfig",OperationConfig)[m];
            hydrated.name = m;

            // flag up an exact name match. Only first exact match counts.
            if (!exactMatchExists) {
                exactMatchExists = _bba‍.a("sanitise",sanitise)(hydrated.name) === _bba‍.a("sanitise",sanitise)(searchTerm);
            }
            // Return hydrated along with what type of match it was
            return {
                hydrated,
                nameExactMatch: _bba‍.a("sanitise",sanitise)(hydrated.name) === _bba‍.a("sanitise",sanitise)(searchTerm),
                nameMatch: _bba‍.a("sanitise",sanitise)(hydrated.name).includes(_bba‍.a("sanitise",sanitise)(searchTerm)),
                descMatch: _bba‍.a("sanitise",sanitise)(hydrated.description).includes(_bba‍.a("sanitise",sanitise)(searchTerm))
            };
        })
 ...
 */
    ...
    const recipe =  new (_bba‍.a("NodeRecipe",NodeRecipe))(recipeConfig);
    const dish = ensureIsDish(input);
 ...
 */
    ...
    */
    const func = () => {
        throw new (_bba‍.a("ExcludedOperationError",ExcludedOperationError))(`Sorry, the ${name} operation is not available in the Node.js version of CyberChef.`);
    };
