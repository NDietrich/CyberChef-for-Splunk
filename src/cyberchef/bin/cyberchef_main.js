...
const getinfo_reply = function(){
	...
	var params = searchOptions.searchinfo.raw_args.join(' ') 
	...
	var requiredFields = [spl_params.inputSearchField]
...
const log = function(msg){
	...
	if((spl_params.debugEnabled == 'full') || (spl_params.debugEnabled == 'info')){
		if (tempLogString && logfile != ''){
			appendFileSync(logfile, tempLogString, function (err) { })
			tempLogString = null

		}
		if(logfile != '' ){
			appendFileSync(logfile, "(PID:" + process.pid + ") "+ msg+ "\n", function (err) { })
		}
...
const halt_on_error = function (msg){
	...
	try {
		...
		var metadata = '{"finished":true,"error":"'  + msg +'"}'
	}
...
const loadRecipesFromFile = function( recipeAliasToLoad ) {
	...
	for(var i=0; i < recipeFolders.length; i++) {
		try {
			var f = readdirSync(recipeFolders[i] )
...
process.stdin.on('readable', () => {
	...
	if(parsedMetadata.action == 'getinfo'){
		...
		// only load modules if we have a valid SID (otherwise it's not needed and will get cancelled by splunk)
		...
		var payload = message.slice(metadataSize)

		var modifiedPayload = modify_payload(payload)
